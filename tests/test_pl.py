...
class TestProfitAndLoss(unittest.TestCase):
    ...
    def test_interest_income_goes_to_other_income(self):
        txs = [
            ...
            make_tx(
                ...
                amount="50.00",
                is_credit=True,
                business_category="Interest Income",
                include_in_pnl=True,
            ),
        ]
        pl = build_pl(txs)
        self.assertEqual(pl.total_revenue, Decimal("1000.00"), "Interest Income should not be in operating revenue")
        self.assertEqual(pl.total_other_income, Decimal("50.00"), "Interest Income should be in other income")
        self.assertEqual(pl.net_profit, Decimal("1050.00"), "Net profit should include other income")
    ...
    def test_other_income_not_counted_as_operating_revenue(self):
        txs = [
            ...
            make_tx(
                ...
                amount="100.00",
                is_credit=True,
                business_category="Other Income",
                include_in_pnl=True,
            ),
        ]
        pl = build_pl(txs)
        self.assertEqual(pl.total_revenue, Decimal("1000.00"), "Other Income should not be operating revenue")
        self.assertNotIn("Other Income", pl.revenue, "Other Income should not appear in revenue dict")
        self.assertIn("Other Income", pl.other_income, "Other Income should appear in other_income dict")
    ...
    def test_loan_interest_goes_to_other_expense(self):
        txs = [
            ...
            make_tx(
                ...
                amount="30.00",
                is_credit=False,
                business_category="Loan Interest",
                include_in_pnl=True,
            ),
        ]
        pl = build_pl(txs)
        self.assertEqual(pl.total_operating_expenses, Decimal("0"), "Loan Interest should not be operating expense")
        self.assertEqual(pl.total_other_expense, Decimal("30.00"), "Loan Interest should be in other expense")
        self.assertEqual(pl.net_profit, Decimal("970.00"), "Net profit deducts other expense")
    ...
    def test_reversal_matches_same_amount_and_nearest_date(self):
        txs = [
            make_tx(
                ...
                amount="900.00",
                is_credit=False,
                business_category="Materials and Supplies",
                include_in_pnl=True,
            ),
            make_tx(
        ...
        pl = build_pl(txs)
        self.assertEqual(
            pl.direct_costs["Materials and Supplies"],
            Decimal("900.00"),
    ...
    def test_reversal_does_not_reuse_matched_debit(self):
        ...
        pl = build_pl(txs)
        self.assertEqual(pl.direct_costs["Fuel"], Decimal("0"), "First $150 reversal should cancel the $150 debit")
        self.assertEqual(
            pl.payment_reversal_credits, Decimal("150.00"), "Second $150 reversal should go to review bucket"
        )
    ...
    def test_reversal_no_close_match_goes_to_review_bucket(self):
        ...
        pl = build_pl(txs)
        self.assertEqual(
            pl.payment_reversal_credits, Decimal("50.00"), "Reversal with no matching debits goes to review bucket"
        )

    def test_reversal_does_not_match_future_debit(self):
        txs = [
            make_tx(
                post_date="01/25/2025",
                description="VENDOR D",
                amount="150.00",
                is_credit=False,
                business_category="Materials and Supplies",
                include_in_pnl=True,
    ...
    def test_reversal_lookback_limit(self):
        txs = [
            make_tx(
                post_date="01/01/2025",
                description="VENDOR F",
                amount="150.00",
                is_credit=False,
                business_category="Materials and Supplies",
                include_in_pnl=True,
